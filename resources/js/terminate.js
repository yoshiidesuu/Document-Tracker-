...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const terminateReasonSection = document.getElementById('terminate-reason-section');
    ...
    function lookupDocument(code) {
        ...
        fetch('/system/documents/terminate/lookup', {
    ...
    function showDocument(data) {
        ...
        const alreadyTerminated = doc.status === 'terminated';

        const ci = document.getElementById('current-holder-info');
        if (alreadyTerminated) {
            ci.innerHTML = '<span class="font-medium text-red-700">Document is already terminated.</span>';
        } else if (data.current_holder) {
            ...
            ci.innerHTML = '<span class="font-medium text-yellow-700">No current holder.</span>';
        }
        ...
        terminateBtn.disabled = alreadyTerminated;
        terminateBtn.textContent = alreadyTerminated ? 'Already Terminated' : 'Terminate Document';
        terminateReasonSection.classList.toggle('hidden', alreadyTerminated);
    ...
    terminateBtn.addEventListener('click', function() {
        ...
        if (!reason) { reasonError.classList.remove('hidden'); terminateReason.focus(); return; }
        reasonError.classList.add('hidden');
        terminateBtn.disabled = true; terminateBtn.textContent = 'Terminating...';
        fetch('/system/documents/terminate/' + currentDocumentId, {
            method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '' },
            body: JSON.stringify({ reason })
        }).then(r => r.json()).then(d => {
            if (d.error) { alert(d.error); terminateBtn.disabled = false; terminateBtn.textContent = 'Terminate Document'; return; }
            alert('Document terminated!'); closeModal.click(); manualCode.value = '';
        }).catch(() => { alert('Error.'); terminateBtn.disabled = false; terminateBtn.textContent = 'Terminate Document'; });
    });
    startScanner();
});
