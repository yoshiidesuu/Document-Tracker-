...
@section('title', \App\Models\SystemSetting::get('site_long_name', config('app.name', 'Document Tracker')) . ' - Generate Document')
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200">
            <h2 class="text-lg font-semibold text-gray-900">New Document</h2>
            <p class="text-sm text-gray-500 mt-1">Fill in the details to generate a new document.</p>
        </div>

        <form method="POST" action="{{ route('system.documents.store') }}" class="px-6 py-5 space-y-5">
            @csrf

            <div>
                <label for="title" class="block text-sm font-medium text-gray-700 mb-1">Document Title <span class="text-red-500">*</span></label>
                <input type="text" id="title" name="title" value="{{ old('title') }}" required maxlength="255" class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all text-sm" placeholder="Enter document title">
            </div>
            ...
            <div>
                ...
                <select id="document_type" name="document_type" required class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all text-sm">
                    ...
                    @foreach ($documentTypes as $dt)
                        <option value="{{ $dt->name }}" @selected(old('document_type') === $dt->name)>{{ $dt->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <select id="arta_setting_id" name="arta_setting_id" required class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all text-sm">
                    @forelse ($artaSettings as $arta)
                        <option value="{{ $arta->id }}" data-days="{{ $arta->days ?? 0 }}" data-hours="{{ $arta->hours ?? 0 }}" data-minutes="{{ $arta->minutes ?? 0 }}" @selected((string) old('arta_setting_id') === (string) $arta->id)>
                            {{ $arta->category }} - {{ $arta->title }} ({{ $arta->duration_label }})
                        </option>
                    @empty
                        <option value="" disabled selected>No active ARTA settings available</option>
                    @endforelse
                </select>
            </div>
            ...
            <div>
                ...
                <input type="text" id="processing_time_display" name="processing_time_display" value="" readonly required class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg text-gray-900 bg-gray-50 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all text-sm" placeholder="Auto-filled from ARTA setting">
                <input type="hidden" id="processing_hours" name="processing_hours" value="{{ old('processing_hours', '0.00') }}">
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select id="is_private" name="is_private" required class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all text-sm">
                        <option value="0" @selected((string) old('is_private', '0') === '0')>Public</option>
                        <option value="1" @selected((string) old('is_private') === '1')>Private</option>
                    </select>
                </div>
                <div>
                    <label for="access_key" class="block text-sm font-medium text-gray-700 mb-1">Access Key</label>
                    <input type="text" id="access_key" name="access_key" value="{{ old('access_key') }}" maxlength="255" class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all text-sm" placeholder="Required if private">
                </div>
            </div>

            <div>
                <label for="notes" class="block text-sm font-medium text-gray-700 mb-1">Notes</label>
                <textarea id="notes" name="notes" rows="3" maxlength="5000" class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all text-sm" placeholder="Optional notes...">{{ old('notes') }}</textarea>
            </div>

            <div class="flex items-center justify-end space-x-3 pt-2">
                <a href="{{ route('system.documents.index') }}" class="px-4 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors">
                    Cancel
                </a>
