...
@section('title', \App\Models\SystemSetting::get('site_long_name', config('app.name', 'Document Tracker')) . ' - My Documents')
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                <h2 class="text-lg font-semibold text-gray-900">My Documents ({{ $documents->count() }})</h2>
                <div class="flex items-center gap-2 w-full sm:w-auto">
                    <form method="GET" action="{{ route('system.documents.my') }}" class="flex items-center gap-2 w-full sm:w-auto">
                        <div class="relative flex-1 sm:flex-initial">
                            ...
                            <input type="text" name="search" placeholder="Search my documents..." value="{{ request('search') }}" class="w-full sm:w-48 pl-9 pr-3 py-1.5 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                        </div>
                        ...
                        @if(request('search') || request('type'))
                            <a href="{{ route('system.documents.my') }}" class="px-3 py-1.5 text-sm font-medium text-gray-600 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors">Clear</a>
                        @endif
                    </form>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm border-collapse border border-gray-300" style="table-layout:fixed">
                ...
                <tbody>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center text-sm text-gray-500 border border-gray-300">You haven't created any documents yet.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
