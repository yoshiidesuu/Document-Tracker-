...
@section('title', \App\Models\SystemSetting::get('site_long_name', config('app.name', 'Document Tracker')) . ' - Offices')
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <h2 class="text-lg font-semibold text-gray-900">All Offices ({{ $offices->count() }})</h2>
                @if(auth()->user()->hasPermission('offices.create'))
                <a href="{{ route('system.offices.create') }}" class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 transition-colors">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($offices as $office)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4">
                                <span class="font-medium text-gray-900">{{ $office->name }}</span>
                            </td>
                            <td class="px-6 py-4">
                                @if($office->code)
                                    <code class="text-xs bg-gray-100 px-2 py-0.5 rounded text-gray-600">{{ $office->code }}</code>
                                @else
                            ...
                            <td class="px-6 py-4 text-right">
                                <button type="button" class="dropdown-toggle inline-flex items-center px-3 py-1.5 text-xs font-medium text-gray-600 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors" data-office-id="{{ $office->id }}">
                                    Actions
                                ...
                                <div class="dropdown-menu hidden fixed w-40 bg-white border border-gray-200 rounded-lg shadow-lg z-[9999] py-1" data-office-id="{{ $office->id }}">
                                    @if(auth()->user()->hasPermission('offices.view'))
                                    <a href="{{ route('system.offices.view', $office->id) }}" class="flex items-center px-4 py-2 text-xs text-gray-700 hover:bg-gray-50 transition-colors">
                                    ...
                                    @endif
                                    @if(auth()->user()->hasPermission('offices.edit'))
                                    <a href="{{ route('system.offices.edit', $office->id) }}" class="flex items-center px-4 py-2 text-xs text-gray-700 hover:bg-gray-50 transition-colors">
                                    ...
                                    <form method="POST" action="{{ route('system.offices.destroy', $office->id) }}" data-confirm="Delete office '{{ $office->name }}'? This cannot be undone.">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center text-sm text-gray-500">No offices found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
