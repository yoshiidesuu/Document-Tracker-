...
@section('title', \App\Models\SystemSetting::get('site_long_name', config('app.name', 'Document Tracker')) . ' - Office Details')
...
<div class="max-w-3xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
        <a href="{{ route('system.offices.index') }}" class="inline-flex items-center text-sm text-gray-500 hover:text-gray-700 transition-colors">
        ...
        <div class="flex items-center gap-2">
            ...
            <a href="{{ route('system.offices.edit', $office->id) }}" class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 transition-colors">
                <svg class="h-3.5 w-3.5 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" /></svg>
                Edit
            </a>
            @endif
            @if(auth()->user()->hasPermission('offices.toggle-status'))
            <form method="POST" action="{{ route('system.offices.toggle-status', $office->id) }}">
                @csrf
                @if ($office->is_active)
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        <div class="px-6 py-5">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div>
                    <span class="text-gray-500">Office Name:</span><br>
                    <span class="font-medium">{{ $office->name }}</span>
                </div>
                <div>
                    <span class="text-gray-500">Code:</span><br>
                    @if($office->code)
                        <code class="text-xs bg-gray-100 px-2 py-0.5 rounded text-gray-600 font-medium">{{ $office->code }}</code>
                    @else
                ...
                <div class="md:col-span-2">
                    ...
                    <span class="text-gray-700">{{ $office->description ?? 'No description provided.' }}</span>
                </div>
                <div>
                    <span class="text-gray-500">Created:</span><br>
                    <span class="font-medium">{{ $office->created_at->format('M d, Y h:i A') }}</span>
                </div>
                <div>
                    <span class="text-gray-500">Last Updated:</span><br>
                    <span class="font-medium">{{ $office->updated_at->format('M d, Y h:i A') }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="mt-6 bg-white rounded-xl border border-red-200 overflow-hidden">
        ...
        <div class="px-6 py-5">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-900">Delete this office</p>
                    <p class="text-xs text-gray-500">Once deleted, this cannot be undone.</p>
                </div>
                <form method="POST" action="{{ route('system.offices.destroy', $office->id) }}" data-confirm="Are you sure you want to permanently delete this office? This cannot be undone.">
                    @csrf
                    @method('DELETE')
