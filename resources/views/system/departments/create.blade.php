...
@section('title', \App\Models\SystemSetting::get('site_long_name', config('app.name', 'Document Tracker')) . ' - Create Department')
...
<div class="max-w-2xl mx-auto">
    ...
    <form method="POST" action="{{ route('system.departments.store') }}" class="space-y-6">
        @csrf
            ...
            <div class="px-6 py-5 space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="code" id="code" value="{{ old('code') }}" pattern="^[a-z0-9-]+$" title="Lowercase letters, numbers, and hyphens only" placeholder="e.g. it-department" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm">
                    </div>
                </div>
        ...
        <div class="flex items-center justify-end space-x-3">
            <a href="{{ route('system.departments.index') }}" class="px-4 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">Cancel</a>
            <button type="submit" class="px-4 py-2.5 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 transition-colors">Create Department</button>
        </div>
    </form>
</div>
@endsection
