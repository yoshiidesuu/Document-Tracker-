...
@section('title', \App\Models\SystemSetting::get('site_long_name', config('app.name', 'Document Tracker')) . ' - Departments')
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <h2 class="text-lg font-semibold text-gray-900">All Departments ({{ $departments->count() }})</h2>
                @if(auth()->user()->hasPermission('departments.create'))
                <a href="{{ route('system.departments.create') }}" class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 transition-colors">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($departments as $department)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                @if($department->code)
                                    <code class="text-xs bg-gray-100 px-2 py-0.5 rounded text-gray-600">{{ $department->code }}</code>
                                @else
                            ...
                            <td class="px-6 py-4 text-right">
                                <button type="button" class="dropdown-toggle inline-flex items-center px-3 py-1.5 text-xs font-medium text-gray-600 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors" data-department-id="{{ $department->id }}">
                                    Actions
                                ...
                                <div class="dropdown-menu hidden fixed w-40 bg-white border border-gray-200 rounded-lg shadow-lg z-[9999] py-1" data-department-id="{{ $department->id }}">
                                    @if(auth()->user()->hasPermission('departments.view'))
                                    <a href="{{ route('system.departments.view', $department->id) }}" class="flex items-center px-4 py-2 text-xs text-gray-700 hover:bg-gray-50 transition-colors">
                                    ...
                                    <a href="{{ route('system.departments.edit', $department->id) }}" class="flex items-center px-4 py-2 text-xs text-gray-700 hover:bg-gray-50 transition-colors">
                                    ...
                                    <form method="POST" action="{{ route('system.departments.destroy', $department->id) }}" data-confirm="Delete department '{{ $department->name }}'? This cannot be undone.">
                                        @csrf
                                        @method('DELETE')
