...
@section('title', \App\Models\SystemSetting::get('site_long_name', config('app.name', 'Document Tracker')) . ' - Roles')
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <h2 class="text-lg font-semibold text-gray-900">All Roles ({{ $roles->count() }})</h2>
                @if(auth()->user()->hasPermission('roles.create'))
                <a href="{{ route('system.roles.create') }}" class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 transition-colors">
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-200">
                        ...
                        <th class="text-center px-6 py-3 font-medium text-gray-500">System</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($roles as $role)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <code class="text-xs bg-gray-100 px-2 py-0.5 rounded text-gray-600">{{ $role->slug }}</code>
                            </td>
                            ...
                            <td class="px-6 py-4 text-center text-sm text-gray-600">
                                {{ $role->permissions ? count($role->permissions) : 0 }}
                            </td>
                            <td class="px-6 py-4 text-right">
                                <button type="button" class="dropdown-toggle inline-flex items-center px-3 py-1.5 text-xs font-medium text-gray-600 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors" data-role-id="{{ $role->id }}">
                                    Actions
                                ...
                                <div class="dropdown-menu hidden fixed w-40 bg-white border border-gray-200 rounded-lg shadow-lg z-[9999] py-1" data-role-id="{{ $role->id }}">
                                        @if(auth()->user()->hasPermission('roles.edit'))
                                        <a href="{{ route('system.roles.edit', $role->id) }}" class="flex items-center px-4 py-2 text-xs text-gray-700 hover:bg-gray-50 transition-colors">
                                            <svg class="h-3.5 w-3.5 mr-2 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" /></svg>
                                            Edit
                                        </a>
                                        @endif
                                        @if(auth()->user()->hasPermission('roles.delete') && !$role->is_system)
                                            <hr class="my-1 border-gray-100">
                                            <form method="POST" action="{{ route('system.roles.destroy', $role->id) }}" data-confirm="Delete role '{{ $role->name }}'? This cannot be undone.">
                                                @csrf
                                                @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center text-sm text-gray-500">No roles found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
